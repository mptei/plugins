    ...
    def parse_item(self, item):
        ...
        if 'hue_dim_max' in item.conf:
            ...
            if not 'hue_dim_step' in item.conf:
                ...
                self.logger.warning('dimmenDPT3: no hue_dim_step defined in item [{0}] using default 25'.format(item.id()))
            if not 'hue_dim_time' in item.conf:
                item.conf['hue_dim_time'] = '1'
                self.logger.warning('dimmenDPT3: no hue_dim_time defined in item [{0}] using default 1'.format(item.id()))
            if not 'hue_dim_min' in item.conf:
                item.conf['hue_dim_min'] = '1'
                self.logger.warning('dimmenDPT3: no hue_dim_min defined in item [{0}] using default 1'.format(item.id()))
            if False:
